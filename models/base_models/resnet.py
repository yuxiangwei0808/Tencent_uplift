"""Copied from https://github.com/median-research-group/LibMTL/blob/main/LibMTL/model/resnet.py#L123"""
...
class ResNet(nn.Module):
    def __init__(self, block, layers, hidden_dim, out_dim=None, in_ch=1, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv1d(in_ch, self.inplanes, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
class ResNetHeavy(nn.Module):
    def __init__(self, block, layers, hidden_dim, out_dim=None, in_ch=1, num_classes=1000, zero_init_residual=False,
        ...
        self.groups = groups
        self.base_width = width_per_group
        self.conv1 = nn.Conv1d(in_ch, self.inplanes, kernel_size=3, padding=1,
                               bias=False)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
...
if __name__ == '__main__':
    x = torch.randn(4, 1, 622)
    model = resnet18(hidden_dim=16, out_dim=38)
    print(model(x).shape)