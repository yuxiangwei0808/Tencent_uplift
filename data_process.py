...
def downsample_to_least(df, target_columns):
    """downsample dataframe according to the least appeared combinations in the target columns"""
...
def stratify_login_days(input_dir):
    ...
    with h5py.File(input_dir + 'dataset_full_0.hdf5', 'r') as file:
    ...
    for k in sampled_data:
        with h5py.File(input_dir + f'dataset_{k}_0.hdf5', 'w') as h5f:
            dataset = h5f.create_dataset('data', data=np.concatenate(sampled_data[k], axis=0), chunks=(3840, 771))


if __name__ == '__main__':
    input_dir = 'data/train_test_data/testdata_240412_240611_zscore/'
    out_file = 'data/train_test_data/traindata_warmtype_240119_240411_zscore/dataset_'
    # bins = group_files_into_bins(input_dir)
    # downsample_func = partial(downsample_untreated_login, col1=614, col2=632)  # used for loginday & next_iswarm
    # downsample_func = partial(downsample_to_least, target_columns=[614, 632, 665])  # used for loginday & next_iswarm & next_9aiwarm
