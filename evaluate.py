...
def valid(model, valid_dataloader, device, metric, num_org_feat):
    ...
    valid_result = {'u_at_k': u_at_k, 'QINI': qini_coef, 'AUUC': uplift_auc, 'WAU': wau}
...
def main(args):    
    ...
    valid_metrics, true_labels, predictions, treatment, add_features = valid(model, valid_dataloader, device, metric, len(org_feat_idx))
